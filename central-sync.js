(() => {
  ...
  const META_KEY = `central-cloud-sync-meta-v1:${APP_ID}`;
  ...
  async function applySnapshot(snapshot) {
    ...
    try {
      ...
      for (const spec of IDB_SPECS) {
        ...
        await idbWrite(spec, Object.prototype.hasOwnProperty.call(remoteIdb, id) ? remoteIdb[id] : (spec.mode === "single" ? null : []));
      }
    } finally {
  ...
  async function pullCloud() {
    ...
    if (meta.payloadHash && hash !== meta.payloadHash) throw new Error("Integridade do backup da nuvem inválida");
  ...
  async function pushCloud() {
    ...
    try {
      ...
      window.dispatchEvent(new CustomEvent("central-sync-error", {
        detail: { appId: APP_ID, error: String(err?.code || err?.message || err) }
      }));
    } finally {
      pushRunning = false;
      if (queuedAfterPush) {
  ...
  async function reconcile() {
    ...
    try {
      ...
      const sameKnownCloud = meta.lastCloudHash && meta.lastCloudHash === remote.hash;
      ...
      const reloadKey = `central-cloud-sync-reloaded-v1:${APP_ID}`;
  ...
  (async () => {
    try {
      // Dá tempo para o auth-guard inicializar o app Firebase padrão.
      await sleep(450);
      ...
      try {
        app = appMod.getApp();
        if (app.options?.projectId !== FIREBASE_CONFIG.projectId) throw new Error("firebase-project-conflict");
      } catch (err) {
      ...
      authMod.onAuthStateChanged(auth, async current => {
        ...
        if (oldUid !== user.uid || initializing) await reconcile();
      });
